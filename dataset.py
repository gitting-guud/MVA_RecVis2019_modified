...
class TrainDataset(Dataset):
    ...
    def __init__(self, phase='train', shape=(64, 64)):
        ...
        self.image_folder = '../bird_dataset/{}_images'.format(self.phase)
        self.label = pd.read_csv('../{}_labels.csv'.format(self.phase), header=None)
...
#                ])
#    def augmenter_func_train(self, image):
...
#        return image
#        
#    def augmenter_func_test(self, image):
##        image = augmenter.augment_image(image)
#        image = ia.imresize_single_image(image, self.shape)
#        image = image / 255
#        image = (image - self.mean) / self.std
...
#    
    def __getitem__(self, index):
        ...
        # load label
        object_class = self.label[1][index].item()
